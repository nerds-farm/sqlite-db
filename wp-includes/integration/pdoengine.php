...
class PDOEngine extends \PDO {
    ...
    public static $dir = SQLITE_DB_PATH . 'wp-includes'.DIRECTORY_SEPARATOR . 'integration' . DIRECTORY_SEPARATOR;
        ...
        if (isset($GLOBALS['@pdo'])) {
            ...
            do {
                try {
                    ...
                    require_once self::$dir . 'pdosqliteudfs.php';
                    new \SQLiteDB\PDOSQLiteUDFS($this->pdo);
    ...
    public function query(string $query, ?int $fetchMode = \PDO::ATTR_DEFAULT_FETCH_MODE, mixed ...$fetchModeArgs) { //: \PDOStatement|false 
        $this->flush();

        //var_dump($query);
        $this->queries[] = "Raw query:\n".$query;
        
        if (substr($query, -7) == " WHERE ") {
            $query .= '1';
        } 
        
        /*if ($query == "SHOW VARIABLES WHERE `Variable_name` IN ( 'version_comment', 'innodb_version' )") {
            if ($fetchMode == 19) {
                //$ret = new stdClass();
                $ret2 = new stdClass();
                $ret2->Value = '1';
                $ret = ['version_comment' => $ret2];
            } else {
                $ret = ['version_comment' => ['Value' => 1]];
            }
            //var_dump($ret);
            return $ret;
        }*/
...
;
                ...
                list($insert, $update) = explode($duplicate_key, $query, 2);
                list($tmp, $key) = explode("`) VALUES ('", $insert, 2);
                list($key, $more) = explode("'", $key, 2);
                var_dump($res);
                var_dump($key);
                var_dump($query); die();*/
                //break;
                list($query, $more) = explode(' ON DUPLICATE KEY UPDATE ', $query, 2);
                //var_dump($query);
            case 'insert':
...
;
    ...
    private function prepare_engine($query_type = null) {
        if (stripos($query_type, 'create') !== false) {
            require_once self::$dir . 'query_create.php';
            $engine = new CreateQuery();
        } elseif (stripos($query_type, 'alter') !== false) {
            require_once self::$dir . 'query_alter.php';
            $engine = new AlterQuery();
        } else {
            require_once self::$dir . 'query.php';
    ...
    private function determine_query_type($query) {
        ...
        $duplicate_key = ' ON DUPLICATE KEY UPDATE';
        if (stripos($query, $duplicate_key) !== false) {
            $this->query_type = 'insert_or_update';
            return true;
        }
    ...
    public function commit(): bool {
        $this->pdo->commit();
        return $this->has_active_transaction = false;
    }

 ...
 */
function sqlite_db_plugin_admin_notice() {

	// Don't print notices in the plugin's admin screen.
	if ( !isset( $_GET['page'] ) || $_GET['page'] != 'sqlite-db' ) {
            return;
	}
	 ...
	 */
	if ( file_exists( WP_CONTENT_DIR . '/db.php' ) && ! defined( 'DB_CONTENT' ) ) {
		printf(
			'<div class="notice notice-error"><p>%s</p></div>',
			sprintf(
	...
	if ( !file_exists( WP_CONTENT_DIR . '/db.php' ) ) {
		...
		printf(
			'<div class="notice notice-error"><p>%s</p></div>',
			esc_html__( 'The SQLite Integration plugin is active, but the wp-content/db.php file is missing and the wp-content directory is not writable. Please ensure the wp-content folder is writable.', 'sqlite-database-integration' )
		);
		return;
            };
            ...
            printf(
                    ...
                    sprintf(
                            ...
                            __( 'The SQLite Integration plugin is active, but the %1$s file is missing.', 'sqlite-database-integration' ),
                            '<code>' . esc_html( basename( WP_CONTENT_DIR ) ) . '/db.php</code>',
...
add_action( 'admin_notices', 'sqlite_db_plugin_admin_notice' ); // Add the admin notices.

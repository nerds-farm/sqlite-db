...
class PatchUtils {
    ...
    private function get_patch_files() {
        $patch_files = array();
        if (!is_dir(SQLITE_PATCH_DIR)) {
            return $patch_files;
        } else {
            if ($dir_handle = opendir(SQLITE_PATCH_DIR)) {
    ...
    private function apply_patches() {
        ...
        foreach ($file_names as $file) {
            ...
            $patch_file = SQLITE_PATCH_DIR . '/' . $file;
     ...
     * temporary directory. If there isn't SQLITE_PATCH_DIR, this method will create it and
    ...
    private function upload_file() {
        ...
        if (!file_exists(SQLITE_PATCH_DIR) || !is_dir(SQLITE_PATCH_DIR)) {
            if (!mkdir(SQLITE_PATCH_DIR, 0707, true)) {
                $message = __('Unable to create a patch directory.', $domain);
                echo '<div id="message" class="updated fade">' . $message . '</div>';
                return false;
            }
        }
        if (!is_file(SQLITE_PATCH_DIR . DIRECTORY_SEPARATOR . '.htaccess')) {
            $fp = fopen(SQLITE_PATCH_DIR . DIRECTORY_SEPARATOR . '.htaccess', 'w');
            if (!$fp) {
        ...
        if (is_uploaded_file($_FILES['upfile']['tmp_name'])) {
            $file_full_path = SQLITE_PATCH_DIR . DIRECTORY_SEPARATOR . $_FILES['upfile']['name'];
            if (move_uploaded_file($_FILES['upfile']['tmp_name'], $file_full_path)) {
                $message = __('File is successfully uploaded.', $domain);
                echo '<div id="message" class="updated fade">' . $message . '</div>';
                chmod(SQLITE_PATCH_DIR . DIRECTORY_SEPARATOR . $_FILES['upfile']['name'], 0606);
            } else {

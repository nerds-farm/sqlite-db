...
class PDOEngine extends PDO {
    ...
    function __destruct() {
        if (defined('SQLITE_MEM_DEBUG') && SQLITE_MEM_DEBUG) {
            ...
            if (is_null($max)) {
                ...
                file_put_contents(DB_PATH . 'mem_debug.txt', $message, FILE_APPEND);
                return true;
            }
    ...
    private function prepare_directory() {
        ...
        if (!is_file(DB_PATH . '.htaccess')) {
            $fh = fopen(DB_PATH . '.htaccess', "w");
            if (!$fh) {
                umask($u);
                $message = 'Unable to create a file in the directory! Please check your server settings.';
                echo $message;
        ...
        if (!is_file(DB_PATH . 'index.php')) {
            $fh = fopen(DB_PATH . 'index.php', "w");
            if (!$fh) {
                umask($u);
                $message = 'Unable to create a file in the directory! Please check your server settings.';
                echo $message;
...
;
        ...
        if (defined('PDO_DEBUG') && PDO_DEBUG === true) {
            file_put_contents(DB_PATH . 'debug.txt', $this->get_debug_info(), FILE_APPEND);
        }
    ...
    private function set_error($line, $function, $message) {
        ...
        if (!$wpdb->show_errors)
            return false;
        file_put_contents(DB_PATH . 'debug.txt', "Line $line, Function: $function, Message: $message \n", FILE_APPEND);
    }
